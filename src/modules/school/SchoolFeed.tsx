...
import type { LessonSummary, LessonType, PlaylistSummary } from "@/shared";
...
type Group = "video" | "reading" | "playlist";

const GROUP_TYPES: Record<"video" | "reading", LessonType[]> = {
  reading: ["article", "pdf"],
  video: ["video"],
};
...
/**
 * Aster School feed: a public, paginated grid of lessons or playlists —
 * Video / Reading / Playlists tabs. Title search and tag filtering apply to
 * lessons only. Logged-in users can add new content. All state changes fetch
 * via the API; loading is set in event handlers so the effect never sets
 * state synchronously.
 */
export function SchoolFeed() {
    ...
    if (filters.group === "playlist") {
      return schoolApi.playlists.list({ page: pageToLoad, limit: LIMIT }).then((res) => {
        if (res.ok) {
          ...
          setPlaylists((prev) =>
            replace ? res.value.playlists : [...prev, ...res.value.playlists],
          );
        } else {
  ...
  const isPlaylistTab = applied.group === "playlist";
  const itemCount = isPlaylistTab ? playlists.length : lessons.length;
  const hasMore = itemCount < total;
    ...
    <main className="relative flex flex-1 flex-col">
      ...
      <div className="relative z-10 mx-auto flex w-full max-w-6xl flex-1 flex-col gap-6 p-6">
        ...
        <header className="flex flex-wrap items-end justify-between gap-4">
          ...
          <Link
            href={isPlaylistTab ? "/school/playlists/new" : "/school/new"}
          ...
          {(["video", "reading", "playlist"] as const).map((g) => (
            <button
              ...
              className={`rounded-full px-4 py-1.5 text-text-sm font-semibold transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-aster-teal-400 ${
                applied.group === g
                  ? "bg-white/10 text-grey-50"
                  : "text-grey-400 hover:text-grey-200"
              }`}
            >
              {g === "reading" ? "Reading" : g === "video" ? "Video" : "Playlists"}
            </button>
          ))}
        </div>
